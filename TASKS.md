...
- **2026-07-29 — live-session detection reads mtimes, not the process table.** Two alternatives were rejected on evidence. A **lockfile written by `cswitch use`** would miss the common case outright: `cswitch aliases` emits `alias claude-x="CLAUDE_CONFIG_DIR=… claude"`, which never runs cswitch, so the guard would report "idle" for the exact sessions it exists to protect — a false negative that costs credentials. A **process-table scan** was rejected as unportable. mtime needs no new dependency and was measured correct against both live profiles.

  **Correction, 2026-07-30 — the portability half of that reasoning was wrong.** `sysinfo` covers Linux, macOS and Windows behind one API, so a process check is perfectly portable; `m2selfA/claude-switch` does exactly that. The argument that actually survives is the same one that killed the lockfile: an alias-launched session never runs `cswitch`, so there is no PID to record and no process to attribute. mtime remains right as the *fallback*, not as the whole answer — see ROADMAP R15 for the both-signals design. The claim also appears in PR #4's description and commit message; corrected there.
